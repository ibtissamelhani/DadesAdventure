<x-user.navbar :experiences="$experiences" :destinations="$destinations">
    <div class="w-full bg-center bg-cover lg:py-10 h-72"
        style="background-image: url('{{ $category->getFirstMediaUrl('cities') }}');">
        <div class="flex items-center justify-center w-full h-full bg-black/50">
            <div class="text-center">
                <h1 class="text-3xl font-semibold text-white lg:text-4xl"><span
                        class="text-cornell-red">{{ $category->name }}</span> Activites</h1>
            </div>
        </div>
    </div>
